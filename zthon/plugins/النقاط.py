...
async def _(event):
    ...
    while estithmar:
        if event.is_group:
            ...
            await asyncio.sleep(3)
            zthon = await event.client.get_messages(event.chat_id, limit=1)
            zthon = zthon[0].message
            zthon = ("".join(zzzthon.split(maxsplit=2)[2:])).split(" ", 2)
            zedub = zthon[0]
            if zedub.isdigit() and int(zedub) > 500000000:
                await event.client.send_message(event.chat_id,f"استثمار {zedub}")
                await asyncio.sleep(5)
                zthon = await event.client.get_messages(event.chat_id, limit=1)
                await zthon[0].click(text="اي ✅")
            else:
                await event.client.send_message(event.chat_id, f"استثمار {zedub}")
...
async def cmd(alsarot):
    await edit_or_reply(alsarot, alsarotCoins_cmd)
