...
zel_dev = (6275274612, 6091420311, 6002442759, 6516959003)
...
async def _(zed):
    await edit_or_reply (zed, "𓆰 [ᯓ 𝙎𝙊𝙐𝙍𝘾𝞝 𝗛𝞝𝗥 - 𝘿𝙀𝙇𝙀𝙏𝙀](https://t.me/HunerThon) 🗑♻️𓆪\n**𓍹━─━─━─━─𝗛𝞝𝗥─━─━─━─━𓍻**\n\n **✵│رابـط الحـذف ↬** https://telegram.org/deactivate \n\n\n **✵│بـوت الحـذف  ↬** @LC6BOT ")
...
async def permalink(mention):
    ...
    if user.id == 6275274612 or user.id == 6516959003 or user.id == 6002442759 or event.chat_id == 6091420311:
...
zedzalTHS_cmd = (
    "𓆩 [𝗦𝗼𝘂𝗿𝗰𝗲 𝗛𝞝𝗥 𝗖𝗼𝗻𝗳𝗶𝗴 𝗩𝗮𝗿𝘀 - اوامـر التحشيش](https://t.me/HunerThon) 𓆪\n\n"
...
async def permalink(mention):
    ...
    if user.id == 6275274612 or user.id == 6275274612 or user.id == 6275274612:
        return await edit_or_reply(mention, f"**╮ ❐ لك هـذا الساروت الحمصي . . شما حجيت ماكـدر اوصفـه 🙊💘 ❏╰**")
    tag = user.first_name.replace("\u2060", "") if user.first_name else user.username
...
async def cmd(alsarot):
    await edit_or_reply(alsarot, alsarotTHS_cmd)
