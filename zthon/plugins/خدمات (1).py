...
async def _(event):
    ...
    async with borg.conversation(chat) as conv: # code by t.me/Y_H_E
        try:
            ...
            alsarot = await conv.get_response()
            if "⏳" not in alsarot.text:
                await alsarot.click(0)
                await asyncio.sleep(5)
                alsarot = await conv.get_response()
                await event.delete()
...
async def _(event):
    ...
    async with borg.conversation(chat) as conv: # code by t.me/Y_H_E
        try:
            ...
            alsarot = await conv.get_response()
            if "Version:" in alsarot.text:
                await alsarot.click(text='Download')
                await asyncio.sleep(5)
                alsarot = await conv.get_response()
                alsarot = alsarot.text
                if "above 50MB" in alsarot.text:
                    aa = alsarot.replace(".apk filesize is above 50MB so you can download only using link", "**- حجم التطبيق اكبر من 50MB ؟!\n- قم بتحميل التطبيق عبـر البوت\n- ادخل للبوت @uploadbot وارسل الرابـط بالاسفـل**\n\n") 
...
async def song2(event):
    ...
    chat = "@apkdl_bot" # code by t.me/Y_H_E
...
async def zed(event):
    ...
    tap = await bot.inline_query(alsarot, zedr)
