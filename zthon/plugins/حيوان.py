...
zed_IMG = sts_animal or sts_animal2 or sts_animal3 or sts_animal4 or sts_animal5 or sts_animal6 or sts_animal7 or sts_animal8 or sts_animal9
...
async def fetch_info(replied_user, event):
    ...
    if x == 1:
       ...
       caption += f"<b> 𓆩 𝙎𝙊𝙐𝙍𝘾𝞝 𝗛𝗨𝗡𝗧𝗘𝗥𝗧𝗛𝗢𝗡 𓆪 </b> - @HunerThon "
...
async def who(event):
    ...
    zed_dev = (6275274612, 6091420311)
    if not os.path.isdir(Config.TMP_DOWNLOAD_DIRECTORY):
        os.makedirs(Config.TMP_DOWNLOAD_DIRECTORY)
    replied_user = await get_user_from_event(event)
    try:
        zed_IMG, caption = await fetch_info(replied_user, event)
    except AttributeError:
        return await edit_or_reply(zed, "**- لـم استطـع العثــور ع الشخــص**")
    if replied_user.id in zed_dev:
       return await edit_or_reply(zed, "**- دي . . انـهُ احـد المطـورين . . انتـه الحيـوان ولك**")
    if replied_user.id == 6275274612 or replied_user.id == 6091420311 or replied_user.id == 6275274612 or replied_user.id == 6516959003
