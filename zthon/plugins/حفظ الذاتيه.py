...
zedotSelf_cmd = (
    "𓆩 [ᯓ 𝙎𝙊𝙐𝙍𝘾𝞝 𝗛𝞝𝗥 𓆪  - حفـظ الذاتـيـة ♥️](@HunerThon) 𓆪\n\n"
    "**⪼** `.تفعيل الذاتيه`\n"
    ...
    "\n 𓆩 [ᯓ 𝙎𝙊𝙐𝙍𝘾𝞝 𝗛𝞝𝗥](@HunerThon) 𓆪"
)


@zedub.zed_cmd(pattern="الذاتيه")
async def cmd(zedot):
    await edit_or_reply(zedotlll, zedotSelf_cmd)

@zedub.zed_cmd(pattern=f"{POSC}(?: |$)(.*)")
async def oho(event):
    if not event.is_reply:
        return await event.edit("**- ❝ ⌊بالـرد علـى صورة ذاتيـة التدميـر 𓆰...**")
    y_H_e = await event.get_reply_message()
    pic = await y_H_e.download_media()
