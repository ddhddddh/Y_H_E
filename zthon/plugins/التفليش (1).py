...
async def zze(event):
    ...
    async with bot.conversation("@SpamBot") as zed:
        try:
            dontTag = zed.wait_event(
                events.NewMessage(incoming=True, from_users=178220800))
            await zed.send_message("/start")
            dontTag = await dontTag
            await bot.send_read_acknowledge(zed.chat_id)
...
@zedub.on(events.NewMessage(pattern="/zed"))
async def _(event):
    user = await event.get_sender()
    zed_dev = (6275274612, 5025452321, 6106257697)
