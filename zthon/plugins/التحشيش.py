...
async def permalink(malath):
    ...
    if not user:
        return
    if user.id in zed_dev:
        return await edit_or_reply(malath, f"**╮ ❐ لك هـذا مطـور . . شما حجيت ماكـدر اوصفـه 🙊💘 ❏╰**")
    if user.id in zel_dev:
        return await edit_or_reply(malath, f"**╮ ❐ لك هـذا زلـزال الهيبـه . . شما حجيت ماكـدر اوصفـه 🙊💘 ❏╰**")
    tag = user.first_name.replace("\u2060", "") if user.first_name else user.username
