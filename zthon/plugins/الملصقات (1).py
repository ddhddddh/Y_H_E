...
EMOJI_SEN = [
    ...
    "Você pode listar vários emojis em uma mensagem, mas recomendo não uzed mais do que dois por cada sticker.",
    "Puoi elencare diverse emoji in un singolo messaggio, ma ti consiglio di non uzedne più di due per sticker.",
    "emoji",
]
...
def pack_name(userid, pack, is_anim, is_video):
    if is_anim:
        return f"HunerThon_{userid}_{pack}_anim"
    if is_video:
        return f"HunerThon_{userid}_{pack}_vid"
    return f"HunerThon_{userid}_{pack}"
