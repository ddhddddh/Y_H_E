...
async def download_audio(event):
    ...
    zedter = await edit_or_reply(event, "**⎉╎جـارِ التحميل انتظر قليلا ▬▭ ...**")
    ...
    for url in urls:
        ...
        for _path in glob.glob(os.path.join(Config.TEMP_DIR, str(startTime), "*")):
            if _path.lower().endswith((".jpg", ".png", ".webp")):
                ...
                _fpath = _path
        if not _fpath:
            return await edit_delete(zedter, "__Unable to upload file__")
        await zedter.edit(
...
async def download_video(event):
    ...
    reply_to_id = await reply_id(event)
    for url in urls:
        ytdl_data = await ytdl_down(zedter, video_opts, url)
        ...
        try:
            ...
            uploaded = await event.client.fast_upload_file(
                ...
                progress_callback=lambda d, t: asyncio.get_event_loop().create_task(
                    progress(
                        d, t, zedter, c_time, "Upload :", file_name=ytdl_data["title"]
                    )
                ),
            )
            ul.close()
...
async def download_video(event):
    ...
    if not urls:
        return await edit_or_reply(event, "**- ضع الرابط مع الامر او بالرد على الرابط ليتم التحميل**")
    zedter = await edit_or_reply(event, "**⎉╎جـارِ التحميل مـن فيـس بـوك انتظر قليلا ▬▭ ...**")
    reply_to_id = await reply_id(event)
    for url in urls:
        ytdl_data = await ytdl_down(zedter, video_opts, url)
        if ytdl_down is None:
...
async def insta_dl(event):
    ...
    async with event.client.conversation(v1) as conv:
        ...
        await edit_or_reply(zedter, "**Switching v2...**")
